    ...
    class AjaxFeeds
      ...
      class LoadResponse
        ...
        def initialize(data)
          ...
          @entries          = []
          feed              = data['feed']
          if @response_status == 200 && Hash === feed
            @title       = (feed['title']      ||'').to_s
            @link        = (feed['link']       ||'').to_s
            @description = (feed['description']||'').to_s
            @author      = (feed['author']     ||'').to_s
            entries      = feed['entries']
            entries.each{|e| @entries << FeedEntry.new(e) } if Array === entries
          end
        end
      ...
      def self.load(url, params = {})
        ...
        json     = Rest.http('ajax.googleapis.com').get('/ajax/services/feed/load', params)
